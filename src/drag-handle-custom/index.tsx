...
export const DragHandleCustom = withDraggableConsumer<IDragHandleCustomProps>(
  class DragHandleCustom extends React.Component<
    IDragHandleCustomProps & IDraggableContext,
    IDragHandleCustomState
  > {
    private events = new SquashEvents();

    constructor(
      props: IDragHandleCustomProps & IDraggableContext,
      context?: any
    ) {
      super(props, context);
      ...
      this.events.push(
        ...
        ({ position }) => {
          this.props.onDragStart({
            position,
            dataMetaOverride: this.onDataMeta()
          });
        }
      );
    }
