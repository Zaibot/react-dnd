...
export const emptyMeasurements: Measurements = {
  ...
  measured: () =>
    warning(
      false,
      `[@zaibot/react-dnd] can't register measurement, measurements root is missing`
    ),
  removed: () =>
    warning(
      false,
      `[@zaibot/react-dnd] can't unregister measurement, measurements root is missing`
    )
};

export const createMeasurementsContext = () =>
  React.createContext(emptyMeasurements);
export const GlobalMeasurementsContext = createMeasurementsContext();

export const MeasurementRoot = ({
  context: { Provider } = GlobalMeasurementsContext,
  children
}: {
  context?: React.Context<Measurements>;
  children: React.ReactNode;
}) => {
  const data = useDictionary();
  const measured = useCallback(
    (key: string, rect: ClientRect) =>
      data.set({ key, value: cloneBounds(rect) }),
    [data.set]
  );
  const removed = useCallback((key: string) => data.remove({ key }), [
    data.remove
  ]);
...
export const withMeasurementRoot = (
  context: React.Context<Measurements> = GlobalMeasurementsContext
) => <P extends {}>(ComponentType: React.ComponentType<P>) => (props: P) => (
  <MeasurementRoot context={context}>
    <ComponentType {...props} />
  </MeasurementRoot>
);
...
export const useMeasureElement = (
  ...
  useEffect(() => {
    const bounds =
      element.current && cloneBounds(element.current.getBoundingClientRect());
    if (bounds) {
...
const cloneBounds = ({
  ...
  width,
  height
}: ClientRect): Bounds => ({ top, left, right, bottom, width, height });
