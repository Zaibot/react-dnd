...
import {
  withDragAndDropData,
  IDraggingProviderRenderProps
} from "../dragging-provider";

export type DragStartHandler = (args: {
  position: IPosition;
  dataMetaOverride?: DataObject;
}) => void;
...
export const withDraggableConsumer = <P extends {}>(
  Component: React.ComponentType<P & IDraggableContext>
): React.ComponentType<P> => {
  return (props: P) => (
    <Consumer>{consumer => <Component {...props} {...consumer} />}</Consumer>
  );
};
export const DraggableContext = withDragAndDropData(
  class DraggableContext extends React.Component<
    IDraggableContextProps & IDraggingProviderRenderProps,
    IDraggableContext
  > {
    public static getDerivedStateFromProps(
      nextProps: IDraggableContextProps & IDraggingProviderRenderProps,
      prevState: IDraggableContext
    ) {
    ...
    constructor(
      props: IDraggableContextProps & IDraggingProviderRenderProps,
      context?: any
    ) {
      super(props, context);
    ...
    private onDragStart: DragStartHandler = args => {
      if (isBoundsSame(this.state.bounds, emptyBounds)) {
        ...
        console.warn(`[@zaibot/react-dnd] missing bounds on draggable`, {
          dataKey,
          dataMeta
        });
      }
    ...
    private onDragEnd: DragHandler = args => {
      ...
      if (this.mounted) {
        this.setState(
          { draggingPosition, isDragging: false, dataMetaOverride: undefined },
          () => {
