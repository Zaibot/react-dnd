...
const calcDistanceTop = (position: IPosition, bounds: IBounds) =>
  pDistance(
    position.left,
    position.top,
    bounds.left,
...
const calcDistanceBottom = (position: IPosition, bounds: IBounds) =>
  pDistance(
    position.left,
    position.top,
    bounds.left,
...
export const vertical = (
  target: IPosition,
  elements: IClosestCornerElement[]
): IClosestCornerResult | null => {
  const foundTop = elements.reduce<IClosestCornerResult | null>(
    (state, element) => {
      const distance = calcDistanceTop(target, element.bounds);
      const isCloserOrNull = !state || distance < state.distance;
      return isCloserOrNull
        ? { element, distance, isBefore: true, isAfter: false }
        : state;
    },
    null
  );
  const foundEither = elements.reduce((state, element) => {
    ...
    const isCloserOrNull = !state || distance < state.distance;
    return isCloserOrNull
      ? { element, distance, isBefore: false, isAfter: true }
      : state;
  }, foundTop);
