...
class ClosestCornerHorizonal extends React.Component {
  ...
  public render() {
    ...
    return <div onMouseMove={this.onMouseMove} style={{ background: `#eee`, padding: `5rem`, width: `10rem`, textAlign: `center`, display: `inline-block` }}>{isOver ? (isBefore ? `Left` : `Right`) : `Hover me`}</div>;
  }
...
class ClosestCornerVertical extends React.Component {
  ...
  public render() {
    const { isOver, isBefore } = this.state;
    return <div onMouseMove={this.onMouseMove} style={{ background: `#eee`, padding: `5rem`, width: `10rem`, textAlign: `center`, display: `inline-block` }}>{isOver ? (isBefore ? `Top` : `Bottom`) : `Hover me`}</div>;
  }
...
class ClosestCornerAll extends React.Component {
  ...
  public render() {
    ...
    return <div onMouseMove={this.onMouseMove} style={{ background: `#eee`, padding: `5rem`, width: `10rem`, textAlign: `center`, display: `inline-block` }}>{isOver ? (`${this.locationToString(location)} @ ${distance.toFixed(1)}px`) : `Hover me`}</div>;
  }
...
stories
  .add(`Horizontal`, () => (
    <ClosestCornerHorizonal />
  ))
  .add(`Vertical`, () => (
    <ClosestCornerVertical />
  ))
  .add(`All`, () => (
    <ClosestCornerAll />
  ));
